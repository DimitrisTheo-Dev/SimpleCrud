...
exports.update = (req, res) => {
    ...
    Tweet.findByIdAndUpdate(req.params.tweetId, {
        ...
        .then(tweet => {
            ...
            if(err.kind === 'ObjectId') {
                return res.status(404).send({
                    message: "Note not found by id " + req.params.tweetId
                });
            }
            return res.status(500).send({
                message: "Error updating the tweet with " + req.params.tweetId
            });
    });
};
...
exports.delete = (req, res) => {
    Tweet.findByIdAndRemove(req.params.tweetId)
        .then(tweet => {
            if(!tweet) {
                return res.status(404).send({
                    message: "Could not find tweet with id " + req.params.tweetId
            ...
            return res.status(500).send({
                message: "Could not delete tweet with id " + req.params.body
            });
    });
